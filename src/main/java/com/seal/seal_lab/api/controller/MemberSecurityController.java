...
public class MemberSecurityController {
    ...
    @GetMapping("/member/security/password")
    public String passwordChangeForm(Authentication authentication, Model model) {
        model.addAttribute("loginId", authentication != null ? authentication.getName() : "member");
        return "auth/member_password_change";
    }

    @PostMapping("/member/security/password")
    public String changePassword(@RequestParam("currentPassword") String currentPassword,
        ...
        try {
            passwordSecurityService.changeMemberPassword(loginId, currentPassword, newPassword, confirmPassword);
        } catch (IllegalArgumentException | IllegalStateException ex) {
            mfaAuditService.recordMemberPasswordChangeFailure(loginId, requestIp, sessionFingerprint, ex.getMessage());
            redirectAttributes.addFlashAttribute("passwordError", ex.getMessage());
            return "redirect:/member/security/password";
        }

        mfaAuditService.recordMemberPasswordChangeSuccess(loginId, requestIp, sessionFingerprint);
        ...
        log.warn("[AUTH-PASSWORD-ROTATED] Member: {} | Current session invalidated after password change.", loginId);
